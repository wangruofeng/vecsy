  ...
  return (
        <section className="canvas-panel">
          <div className="canvas-toolbar">
            ...
            {activeTab !== 'source' && <div className="view-tabs canvas-mode-tools">{[['select', copy.canvasSelect, 'V', 'cursor'], ['pan', copy.canvasPan, 'H', 'hand']].map(([tool, label, shortcut, icon]) => <button key={tool} className={canvasTool === tool ? 'active' : ''} type="button" title={`${label} (${shortcut})`} aria-keyshortcuts={shortcut} aria-pressed={canvasTool === tool} onClick={() => setCanvasTool(tool)}><Icon name={icon} size={14} /> {label}</button>)}</div>}
