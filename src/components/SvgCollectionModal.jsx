  ...
  return <div className="shortcuts-overlay svg-collection-overlay" onClick={onClose}>
    <div className="shortcuts-modal svg-collection-modal" role="dialog" aria-modal="true" aria-label={copy.svgCollectionTitle} onClick={(event) => event.stopPropagation()}>
      ...
      <div className="svg-collection-body">
        ...
        <div className="svg-collection-content"><div className="svg-collection-heading"><div><strong>{copy[activeCollection.labelKey]}</strong><span>{activeCollection.items.length} {copy.svgCollectionItems}</span></div><a href={activeCollection.sourceUrl} target="_blank" rel="noreferrer">{copy.svgCollectionSource} ↗</a></div><div className="svg-collection-grid">{activeCollection.items.map((item) => <button key={item.id} className="svg-collection-item" type="button" title={`${copy.svgCollectionAdd} ${item.name}`} onClick={() => onSelect(item)}><span className="svg-collection-preview"><img src={item.url} alt="" /></span><span>{item.name}</span></button>)}</div></div>
      </div>
    </div>
  </div>
}
