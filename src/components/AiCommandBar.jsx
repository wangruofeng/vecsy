...
const ERROR_COPY_KEYS = {
  ...
  AI_REQUEST_FAILED: 'aiErrorRequestFailed',
  RATE_LIMITED: 'aiErrorRateLimited',
  TIMEOUT: 'aiErrorTimeout',
}

export default function AiCommandBar({ copy, aiDesign, selectedIds, onOpenSettings }) {
  const [prompt, setPrompt] = useState('')
  const [jsonOpen, setJsonOpen] = useState(false)
  const [jsonDraft, setJsonDraft] = useState('')
  const busy = aiDesign.status === 'thinking' || aiDesign.status === 'validating' || aiDesign.status === 'applying'
  ...
  const needsSetup = !aiDesign.configured

  return <div className="ai-command-dock">
    <div className="ai-command-meta">
      <span>{copy.aiRuntimeLabel}</span>
      <div className="ai-command-meta-actions">
        <button type="button" onClick={() => setJsonOpen((current) => !current)} aria-expanded={jsonOpen}>{copy.aiJsonDebug}</button>
        <button type="button" className="ai-settings-button" onClick={onOpenSettings}>{copy.aiOpenSettings}</button>
      </div>
    </div>
    {needsSetup && <div className="ai-setup-notice" role="status">
      <span>{copy.aiSettingsRequired}</span>
      <button type="button" className="button button-accent" onClick={onOpenSettings}>{copy.aiOpenSettings}</button>
    </div>}
    ...
    <form className="ai-command-form" onSubmit={submitPrompt}>
      <input value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder={copy.aiCommandPlaceholder} disabled={busy || !selectedIds.length || needsSetup} aria-label={copy.aiCommandPlaceholder} />
      <button className="button button-accent" type="submit" disabled={busy || !selectedIds.length || needsSetup}>{copy.aiPreview}</button>
    </form>
    ...
    {busy && <span className="ai-status">{aiDesign.status === 'applying' ? copy.aiStatusApplying : aiDesign.status === 'thinking' ? copy.aiStatusThinking : copy.aiStatusValidating}</span>}
    {error && <p className="ai-error" role="alert">{error}<button type="button" className="ai-retry" onClick={aiDesign.retry}>{copy.aiRetry}</button></p>}
