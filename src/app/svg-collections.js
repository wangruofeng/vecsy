const COMPANY_LOGOS = [
  'Digg', 'Deviantart', 'Blogger', 'Dribbble', 'Apple', 'Behance', 'Flickr', 'Ebay', 'Dropbox', 'Facebook', 'Forrst', 'Github', 'Instagram', 'Gplus', 'Lastfm', 'Gdrive', 'Picasa', 'Linkedin', 'Pinterest', 'Skype', 'Reddit', 'Rss', 'Stumbleupon', 'Soundcloud', 'Tumblr', 'Vimeo', 'Vine', 'Twitter', 'Wordpress', 'Yahoo', 'Amazon', 'Android', 'Windows', 'Youtube',
]
...
export const SVG_COLLECTIONS = [{
  ...
  items: COMPANY_LOGOS.map((name) => {
    const url = getCollectionAssetUrl('company-logos', `${name.toLowerCase()}.svg`)
    return { id: `company-logo-${name.toLowerCase()}`, name, url, editableUrl: url }
  }),
