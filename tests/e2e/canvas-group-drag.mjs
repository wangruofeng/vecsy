// Run against a local dev server: node tests/e2e/canvas-group-drag.mjs http://127.0.0.1:5186
...
try {
  ...
  await page.locator('input[type=file][accept="image/svg+xml,.svg"]').setInputFiles({ name: 'group-drag.svg', mimeType: 'image/svg+xml', buffer: Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300"><rect id="background" y="100" width="400" height="180" fill="#aaa"/><g id="foreground" transform="translate(10 5)"><rect id="first" x="50" y="80" width="50" height="80" fill="red"/><rect id="second" x="220" y="80" width="50" height="80" fill="blue"/></g></svg>') })
  ...
  const group = page.locator('.svg-wrap #foreground')
  const groupId = await group.getAttribute('data-editor-id')
  await page.locator(`.layer-row[data-layer-id="${groupId}"] .layer-name`).click()
  await settle()
  assert.equal(await page.locator('.layer-row.selected').count(), 1, 'panel selects the group ID')
  ...
  const assertMove = async (dx, dy, fromChild = false) => {
    ...
    await drag(fromChild ? before[0].x + before[0].width / 2 : gapX, gapY, dx, dy)
  ...
  await page.screenshot({ path: '/tmp/vecsy-group-drag.png' })
  ...
  console.log('Group drag E2E passed: panel selection, group gap, child hit, transformed group, undo, zoom, outside selection; no page errors')
